...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!csrf_verify($_POST['csrf_token'] ?? null)) {
        ...
        redirect(url('pages/ingredient/edit.php?id=' . $id));
    }
...
$pageTitle = 'Edit · ' . ($ingredient['ingredient_name'] ?? '');
component('layout/header', ['pageTitle' => $pageTitle]);
component('layout/main',   ['title' => 'Edit Ingredient', 'crumbs' => ['Stock', 'Ingredients', 'Edit']]);
component('inventory/ingredient-form', [
    'ingredient' => $ingredient,
    'suppliers'  => $suppliers,
    'errors'     => $errors ?? [],
    'action'     => url('pages/ingredient/edit.php?id=' . $id),
]);
