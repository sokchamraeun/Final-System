 ...
 * $delta_qty > 0  â†’ more drinks: deduct (honest â€” floors at 0, logs actual, returns shortfalls).
 * $delta_qty < 0  â†’ fewer/removed drinks: restore the difference.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'save') {
    ...
    try {
        // â”€â”€ STOCK SYNC: restore/deduct ingredient stock for the edit delta â”€â”€
        // Snapshot current quantities BEFORE mutating, then for each item compute
        // new-vs-old: removed â†’ restore full, reduced â†’ restore diff, increased â†’ deduct diff.
        $pre = [];
        ...
        // â”€â”€ LOYALTY: keep points in sync with the new drink count â”€â”€
        ...
        if ($lc_id > 0) {
            ...
            if ($delta !== 0) {
                ...
                $hdesc = "Order #{$order_id} edited â€” points adjusted by {$delta}";
        ...
        if (!empty($stock_warnings)) {
            ...
            $warning = 'Low stock â€” ' . implode(', ', $names) . ' ran short. Restock soon.';
        }
        ...
        <div class="panel">
            ...
            <div class="item-row" id="row-<?= $item['item_id'] ?>" data-id="<?= $item['item_id'] ?>" data-price="<?= (float)$item['price'] ?>">
                ...
                <div class="item-info">
                    ...
                    <?php if ($customs): ?>
                    <div class="item-custom"><?= htmlspecialchars(implode(' Â· ', $customs)) ?></div>
                    <?php endif; ?>
                ...
                <div class="item-controls">
                    <div class="qty-control">
                        <button type="button" onclick="changeQty(<?= $item['item_id'] ?>, -1)" id="minus-<?= $item['item_id'] ?>">âˆ’</button>
...
async function saveChanges() {
    ...
    try {
        ...
        if (data.success) {
            ...
            updateItemCount();
            if (data.warning) {
                showToast('Order updated â€” new total $' + data.total, 'success');
