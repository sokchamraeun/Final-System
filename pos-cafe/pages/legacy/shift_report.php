...
// â”€â”€ Orders taken this shift by this user â”€â”€
$stmt = $conn->prepare("
...
// â”€â”€ Drinks sold by type this shift â”€â”€
$drinks = [];
$total_drinks = 0;
if ($order_ids) {
...
// â”€â”€ Inventory-clerk shift summary (stock activity instead of orders) â”€â”€
...
// â”€â”€ AJAX: save cash reconciliation â”€â”€
...
<title>Shift Report â€” <?= h($today_fmt) ?></title>
...
/* â”€â”€ Actions (admin/manager only) â”€â”€ */
...
<div class="stat-grid">
<?php if ($is_clerk):
    $sc_txt = $sc_status === 'submitted' ? 'Done' : ($sc_status === 'draft' ? 'Open' : 'â€”');
?>
...
<div class="section s2" style="animation-delay:.48s">
    ...
    <div class="drinks-grid">
        ...
        <div class="drink-row">
            ...
            <span class="drink-qty"><?= (int)$d['qty'] ?>Ã—</span>
        </div>
        <?php endforeach; ?>
    </div>
...
<?php else: ?>
    ...
    <div class="order-rows">
        ...
        <div class="order-row">
            ...
            <span class="amount"><?= in_array($status,['Cancelled','Refunded']) ? '<span style="color:#444">â€”</span>' : '$'.number_format((float)$o['total'],2) ?></span>
...
<div class="actions">
    <a href="/FinalSystem/pos-cafe/pages/dashboard/index.php" class="btn btn-ghost"><i class="fa-solid fa-arrow-left"></i> Back</a>
...
(function () {
    ...
    // No cash step â€” straight farewell + logout
    startFarewell();
    <?php endif; ?>
    ...
    window.submitCash = async function () {
        ...
        if (isMatch || attempt >= 1) {
            ...
            if (isMatch) {
                ...
                result.textContent = 'âœ“ Drawer balanced. Great job!';
            } else {
                result.className = 'cash-result discrepancy';
                const dir = diff > 0 ? 'over' : 'short';
                result.textContent = `âš  Still ${dir} by $${Math.abs(diff).toFixed(2)}. Discrepancy logged â€” manager will review.`;
            }
            setTimeout(startFarewell, 2000);

        } else {
            // First attempt, off â€” show recount option, do NOT save yet
            attempt = 1;
            result.style.display = 'block';
            result.className = 'cash-result warn';
            const dir = diff > 0 ? 'over' : 'short';
            result.textContent = `â†• Drawer is ${dir} by $${Math.abs(diff).toFixed(2)}. Please recount your drawer.`;
