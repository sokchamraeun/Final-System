...
// â”€â”€ AJAX: return preparing orders as JSON â”€â”€
if (isset($_GET['ajax'])) {
    $stmt = $conn->prepare("
         SELECT o.order_id, o.daily_order_no, o.customer_name, o.order_date,
                oi.product_name, oi.quantity, oi.sweetness, oi.ice, oi.sugar, oi.milk, oi.size_label
        FROM orders o
...
<div class="topbar">
    <div class="topbar-left">
        ...
        <div>
            <div class="topbar-logo">â˜• Barista Station</div>
...
function buildMods(item) {
    ...
    if (item.sugar)     mods.push('Sugar: ' + item.sugar);
...
function renderTicket(o) {
    const items = o.items.map(item => {
        ...
        return `<div class="ticket-item">
            <div class="item-qty">${esc(item.qty)}Ã—</div>
            <div class="item-body">
...
async function markDone(orderId, card) {
    ...
    try {
        ...
        // Server rejected (or unreachable) â€” keep the ticket on screen, surface why
