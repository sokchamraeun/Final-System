...
/* Categories — list / search / paginate. */
...
$canManage = can('categories');

$search = (string) input('search', '');
$page   = max(1, (int) input('page', 1));

$category = new Category();
$result   = $category->paginate(['search' => $search], $page);

$db          = Database::instance();
$totalCats   = (int) $db->scalar("SELECT COUNT(*) FROM categories");
$withImg     = (int) $db->scalar("SELECT COUNT(*) FROM categories WHERE image IS NOT NULL AND image != ''");
$hasProducts = (int) $db->scalar("SELECT COUNT(DISTINCT category_id) FROM products WHERE category_id IS NOT NULL");
$emptyCats   = $totalCats - $hasProducts;

$stats = [
    ['label' => 'Total Categories', 'value' => $totalCats, 'icon' => 'fa-tag', 'color' => 'text-brand', 'bg' => 'bg-amber-50 dark:bg-slate-800'],
    ['label' => 'With Image',       'value' => $withImg,   'icon' => 'fa-image', 'color' => 'text-blue-600', 'bg' => 'bg-blue-50 dark:bg-slate-800'],
    ['label' => 'Has Products',     'value' => $hasProducts, 'icon' => 'fa-mug-hot', 'color' => 'text-emerald-600', 'bg' => 'bg-emerald-50 dark:bg-slate-800'],
    ['label' => 'Empty Categories', 'value' => $emptyCats, 'icon' => 'fa-folder-open', 'color' => 'text-slate-500', 'bg' => 'bg-slate-50 dark:bg-slate-800'],
];
...
$actions = $canManage
    ? '<a href="' . e(url('pages/categories/create.php')) . '" class="inline-flex items-center gap-2 rounded-xl bg-brand px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-brand-600"><i class="fa-solid fa-plus"></i> New Category</a>'
    : '';

$baseUrl = url('pages/categories/index.php?search=' . urlencode($search));

component('layout/header', ['pageTitle' => $pageTitle, 'pageSubtitle' => $pageSubtitle]);
component('layout/main',   ['title' => 'Categories', 'crumbs' => ['Catalog', 'Categories'], 'actions' => $actions]); ?>
...
<form method="GET" class="mb-5 flex items-center gap-3">
  ...
  <?php if ($search !== ''): ?>
    <a href="<?= e(url('pages/categories/index.php')) ?>" class="rounded-xl bg-slate-100 px-4 py-2.5 text-sm font-semibold text-slate-600 transition hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-300">Clear</a>
  <?php endif; ?>
</form>
<?php

component('categories/category-table', ['rows' => $result['rows'], 'canManage' => $canManage]);
...
if ($canManage):
    ...
    ob_start(); ?>
    <form method="POST" action="<?= e(url('pages/categories/delete.php')) ?>">
      <?= csrf_field() ?>
    ...
    component('common/modal', [
        ...
        'body'   => '<p>You are about to delete <strong id="deleteCategoryName"></strong>. This cannot be undone.</p>',
        'footer' => $footerHtml,
    ]);
endif;
?>
