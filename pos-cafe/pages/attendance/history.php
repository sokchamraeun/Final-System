...
function qs(array $overrides = []): string {
    ...
    return 'history.php?' . http_build_query($p);
}
...
<style>
:root{--accent:#d1904b;--accent-light:#e8b87a;--accent-dark:#a0702a;--bg:#0b0b0b;--card:#111;--border:rgba(255,255,255,0.07);--text:#f5f5f5;--text-muted:#777;--success:#55e087;--warning:#f39c12;--danger:#e74c3c;}
[data-theme="light"]{--bg:#f0f2f5;--card:#fff;--border:rgba(0,0,0,0.08);--text:#111;--text-muted:#6b7280;}
[data-theme="light"] .emp-dd-panel{background:#fff;border-color:#e5e7eb;}
[data-theme="light"] .emp-dd-search{border-color:#e5e7eb;}
[data-theme="light"] .date-input{color-scheme:light;}
[data-theme="light"] .back-btn{background:rgba(209,144,75,.1);}
[data-theme="light"] input,[data-theme="light"] .date-input{color:#111;background:rgba(0,0,0,.02);}
*,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
body{font-family:'Poppins',sans-serif;background:var(--bg);color:var(--text);min-height:100vh;}
::-webkit-scrollbar{width:5px;height:5px;}
::-webkit-scrollbar-thumb{background:var(--accent);border-radius:10px;}
[data-theme="light"] ::-webkit-scrollbar-thumb{background:var(--accent-dark);}
.topbar{position:sticky;top:0;z-index:100;background:rgba(10,10,10,.95);border-bottom:1px solid var(--border);backdrop-filter:blur(12px);display:flex;align-items:center;justify-content:space-between;padding:0 28px;height:62px;}
.topbar-left{display:flex;align-items:center;gap:16px;}
[data-theme="light"] .topbar{background:rgba(255,255,255,.95);}
.back-btn{display:inline-flex;align-items:center;gap:7px;text-decoration:none;color:#d1904b;font-size:13px;font-weight:600;padding:7px 14px;border-radius:10px;border:1px solid rgba(209,144,75,.35);background:rgba(209,144,75,.08);transition:all .2s;}
...
.date-nav a{display:inline-flex;align-items:center;gap:6px;padding:7px 14px;border-radius:8px;border:1px solid var(--border);background:rgba(255,255,255,.03);color:var(--text-muted);text-decoration:none;font-size:13px;transition:all .2s;}
.date-nav a:hover{color:var(--accent);border-color:rgba(209,144,75,.3);}
.date-input{padding:7px 14px;border-radius:8px;border:1px solid var(--border);background:rgba(255,255,255,.03);color:var(--text);font-size:13px;font-family:'Poppins',sans-serif;outline:none;transition:border-color .2s;color-scheme:dark;}
.date-input:focus{border-color:rgba(209,144,75,.45);}
.date-input option{background:#1a1a1a;color:#f5f5f5;}
[data-theme="light"] .date-input option{background:#fff;color:#111;}
...
.date-label{font-size:15px;font-weight:700;color:var(--text);}
...
.card-icon{width:38px;height:38px;border-radius:10px;flex-shrink:0;background:linear-gradient(135deg,rgba(209,144,75,.2),rgba(209,144,75,.06));border:1px solid rgba(209,144,75,.2);display:flex;align-items:center;justify-content:center;color:var(--accent);font-size:15px;}
.card-title{font-size:14px;font-weight:700;}
.card-sub{font-size:12px;color:var(--text-muted);margin-top:2px;}
table{width:100%;border-collapse:collapse;}
thead th{padding:11px 20px;text-align:left;font-size:10.5px;font-weight:700;text-transform:uppercase;letter-spacing:.6px;color:var(--text-muted);border-bottom:1px solid var(--border);background:rgba(255,255,255,.02);}
...
.scard{animation:fadeUp .4s cubic-bezier(.22,1,.36,1) both;transition:transform .22s ease,box-shadow .22s ease,border-color .22s ease;}
...
.preset-btn{display:inline-flex;align-items:center;height:38px;padding:0 14px;border-radius:8px;border:1px solid var(--border);background:rgba(255,255,255,.03);color:var(--text-muted);font-size:12.5px;font-weight:600;text-decoration:none;transition:all .2s;}
...
.csv-btn{display:inline-flex;align-items:center;gap:7px;height:38px;padding:0 16px;border-radius:8px;border:1px solid rgba(85,224,135,.3);background:rgba(85,224,135,.08);color:var(--success);font-size:12.5px;font-weight:600;text-decoration:none;transition:all .2s;}
...
<div class="topbar">
    <div class="topbar-left">
        <a href="<?= url('attendance') ?>" class="back-btn"><i class="fa-solid fa-arrow-left"></i> Attendance</a>
