...
/* Category table. component('categories/category-table', ['rows' => $rows, 'canManage' => bool]); */
$rows      = $rows      ?? [];
...
if (!$rows) {
    component('common/empty-state', [
        'icon'    => 'fa-tags',
        'title'   => 'No categories',
        'message' => 'Create categories to organise your products.',
    ]);
    return;
}
?>
  ...
  <table class="w-full text-sm">
    ...
    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
      ...
      <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50">
        <td class="px-5 py-3 font-medium text-slate-700 dark:text-slate-200"><?= (int) ($r['category_id'] ?? $r['id'] ?? 0) ?></td>
        <td class="px-5 py-3">
        ...
        <td class="px-5 py-3 text-right">
          <a href="<?= e(url('pages/categories/edit.php?id=' . ((int) ($r['category_id'] ?? $r['id'] ?? 0)))) ?>" class="inline-flex items-center gap-1 rounded-lg px-3 py-1.5 text-xs font-semibold text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800">
            <i class="fa-solid fa-pen"></i> Edit
          </a>
          <button onclick="confirmDeleteCategory(<?= (int) ($r['category_id'] ?? $r['id'] ?? 0) ?>, '<?= e($r['name'] ?? '') ?>')" class="inline-flex items-center gap-1 rounded-lg px-3 py-1.5 text-xs font-semibold text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20">
