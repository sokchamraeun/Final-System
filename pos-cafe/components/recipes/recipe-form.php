...
/* Recipe assign/edit form. component('recipes/recipe-form', ['action'=>url(...), 'recipe'=>[], 'products'=>[], 'ingredients'=>[], 'errors'=>[]]); */
$recipe      = $recipe      ?? [];
$products    = $products    ?? [];
$ingredients = $ingredients ?? [];
$errors      = $errors      ?? [];
$action      = $action      ?? '';
$isEdit      = !empty($recipe['id']);

$val = static fn(string $k, $d = '') => e($recipe[$k] ?? $d);
?>
<form method="POST" action="<?= e($action) ?>" class="space-y-5 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
  ...
  <div class="grid gap-5 sm:grid-cols-2">
    <div>
      <label class="mb-1.5 block text-sm font-medium text-slate-600 dark:text-slate-300">Product</label>
      <select name="product_id" <?= $isEdit ? 'disabled' : 'required' ?>
        ...
        <?php foreach ($products as $p): ?>
          <option value="<?= (int) $p['product_id'] ?>" <?= (int) ($recipe['product_id'] ?? 0) === (int) $p['product_id'] ? 'selected' : '' ?>>
            <?= e($p['name']) ?>
          </option>
        <?php endforeach; ?>
      </select>
      <?php if ($isEdit): ?>
        <input type="hidden" name="product_id" value="<?= (int) $recipe['product_id'] ?>">
      <?php endif; ?>
    </div>
    ...
    <div>
      ...
      <select name="ingredient_id" <?= $isEdit ? 'disabled' : 'required'
        ...
        <?php foreach ($ingredients as $id => $name): ?>
          <option value="<?= $id ?>" <?= (int) ($recipe['ingredient_id'] ?? 0) === $id ? 'selected' : '' ?>>
            <?= e($name) ?>
          </option>
        <?php endforeach; ?>
      </select>
      <?php if ($isEdit): ?>
        <input type="hidden" name="ingredient_id" value="<?= (int) $recipe['ingredient_id'] ?>">
      <?php endif; ?>
    </div>
  ...
  <div class="flex items-center justify-end gap-2 border-t border-slate-100 pt-5 dark:border-slate-800">
    <a href="<?= e(url('pages/recipes/index.php')) ?>" class="rounded-xl bg-slate-100 px-5 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-200">Cancel</a>
