...
// Ã¢â€â‚¬Ã¢â€â‚¬ Daily sales target (configurable in Settings Ã¢â€ â€™ Sales Target) Ã¢â€â‚¬Ã¢â€â‚¬
$dailyTarget = DAILY_SALES_TARGET;

// Ã¢â€â‚¬Ã¢â€â‚¬ Is this period "live" (includes today)? Ã¢â€â‚¬Ã¢â€â‚¬
...
<style>
/* ── REPORT PAGE: Dashboard theme ── */
:root {
    --rp-bg: #f1f5f9;
    --rp-card: #ffffff;
    --rp-border: #e2e8f0;
    --rp-border-hover: #d1904b;
    --rp-text: #1e293b;
    ...
    --rp-warn: #eab308;
    --rp-amber: #d1904b;
    --rp-shadow: 0 1px 3px rgba(0,0,0,0.06);
...
[data-theme="dark"] {
    ...
    --rp-shadow-lg: 0 4px 12px rgba(0,0,0,0.4);
}

body { background: var(--rp-bg); color: var(--rp-text); font-family: 'Inter', system-ui, sans-serif; }

/* ── Top Header ── */
.top { display: flex; flex-wrap: wrap; align-items: center; gap: 12px; margin-bottom: 24px; }
.top h2 { font-size: 24px; font-weight: 800; color: #0f172a; margin: 0; display: flex; align-items: center; gap: 10px; }
[data-theme="dark"] .top h2 { color: #fff; }
.report-date { font-size: 13px; color: var(--rp-text-muted); display: flex; align-items: center; gap: 6px; }
.top > div:last-child { margin-left: auto; display: flex; gap: 8px; align-items: center; flex-wrap: wrap; }
a.back { display: inline-flex; align-items: center; gap: 6px; font-size: 13px; font-weight: 500; color: var(--rp-text-muted); text-decoration: none; transition: color 0.2s; }
a.back:hover { color: var(--rp-accent); }

.btn-generate, .theme-toggle {
    display: inline-flex; align-items: center; gap: 6px; padding: 8px 16px; border-radius: 10px;
    font-size: 13px; font-weight: 600; border: 1px solid var(--rp-border);
    background: var(--rp-card); color: var(--rp-text); cursor: pointer; transition: all 0.2s;
}
.btn-generate:hover, .theme-toggle:hover { border-color: var(--rp-accent); color: var(--rp-accent); }

/* ── Card ── */
.card { background: var(--rp-card); border: 1px solid var(--rp-border); border-radius: 16px; padding: 24px; box-shadow: var(--rp-shadow); margin-bottom: 20px; transition: all 0.2s ease; }
.card:hover { border-color: var(--rp-border-hover); box-shadow: var(--rp-shadow-lg); }

/* ── Filters ── */
.filters { display: flex; flex-wrap: wrap; gap: 12px; align-items: flex-end; }
.filters > div { display: flex; flex-direction: column; gap: 4px; }
.filters label { font-size: 12px; font-weight: 600; color: var(--rp-text-muted); text-transform: uppercase; letter-spacing: 0.5px; }
.filters select, .filters input { padding: 8px 12px; border-radius: 10px; border: 1px solid var(--rp-border); background: var(--rp-card); color: var(--rp-text); font-size: 14px; min-width: 160px; transition: border-color 0.2s; }
.filters select:focus, .filters input:focus { outline: none; border-color: var(--rp-accent); }
.filters .btn { display: inline-flex; align-items: center; gap: 6px; padding: 8px 20px; border-radius: 10px; font-size: 14px; font-weight: 600; background: var(--rp-accent); color: #fff; border: none; cursor: pointer; transition: all 0.2s; }
.filters .btn:hover { opacity: 0.9; }

/* ── KPI Grid ── */
.kpis { display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 16px; margin-top: 20px; }
.kpi { background: var(--rp-card); border: 1px solid var(--rp-border); border-radius: 16px; padding: 20px; box-shadow: var(--rp-shadow); transition: all 0.2s ease; position: relative; overflow: hidden; }
.kpi:hover { border-color: var(--rp-border-hover); box-shadow: var(--rp-shadow-lg); transform: translateY(-2px); }
.kpi-icon { width: 40px; height: 40px; border-radius: 12px; display: grid; place-items: center; font-size: 16px; background: rgba(209,144,75,0.1); color: var(--rp-accent); margin-bottom: 12px; }
.kpi span { font-size: 12px; font-weight: 600; color: var(--rp-text-muted); text-transform: uppercase; letter-spacing: 0.5px; display: block; margin-bottom: 4px; }
.kpi h3.num { font-size: 28px; font-weight: 800; color: #0f172a; margin: 0; line-height: 1.1; }
[data-theme="dark"] .kpi h3.num { color: #fff; }
.kpi .small { font-size: 12px; color: var(--rp-text-muted); margin-top: 4px; }
.kpi-delta { font-size: 11px; margin-top: 4px; font-weight: 600; }

/* ── Goal Bar ── */
.goal-bar-wrap { margin-top: 8px; }
.goal-bar-track { height: 5px; background: rgba(209,144,75,0.12); border-radius: 99px; overflow: hidden; margin-bottom: 4px; }
.goal-bar-fill { height: 100%; background: var(--rp-accent); border-radius: 99px; transition: width 0.6s ease; }
.goal-label { font-size: 11px; color: var(--rp-text-muted); }

/* ── Live bar ── */
.live-bar { display: flex; align-items: center; gap: 8px; font-size: 11px; color: var(--rp-text-muted); margin: 10px 0 0; padding: 6px 14px; background: rgba(34,197,94,0.06); border: 1px solid rgba(34,197,94,0.15); border-radius: 50px; width: fit-content; }
.live-dot { width: 7px; height: 7px; border-radius: 50%; background: var(--rp-pos); flex-shrink: 0; }
...
/* ── Insights ── */
.insights-row { display: flex; flex-wrap: wrap; gap: 10px; margin: 16px 0; }
.insight-chip { display: flex; align-items: center; gap: 10px; padding: 10px 16px; border-radius: 12px; background: var(--rp-card); border: 1px solid var(--rp-border); font-size: 13px; transition: all 0.2s; }
.insight-chip:hover { border-color: var(--rp-border-hover); }
.insight-chip i { color: var(--rp-accent); font-size: 16px; }
.ic-label { font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: var(--rp-text-muted); }
.ic-val { font-weight: 600; color: var(--rp-text); }
.ic-good i { color: var(--rp-pos); }
.ic-warn i { color: var(--rp-warn); }
.ic-alert i { color: var(--rp-neg); }

/* ── Section header ── */
.section-hdr { margin-bottom: 16px; }
.section-hdr-row { display: flex; align-items: center; gap: 8px; }
.section-hdr-title { font-size: 16px; font-weight: 700; color: #0f172a; }
[data-theme="dark"] .section-hdr-title { color: #fff; }
.section-hdr i { color: var(--rp-accent); }
.section-hdr-badge { font-size: 11px; padding: 2px 8px; border-radius: 20px; background: rgba(209,144,75,0.1); color: var(--rp-accent); font-weight: 600; }
.section-desc { font-size: 13px; color: var(--rp-text-muted); margin: 4px 0 0; }

/* ── Chart ── */
.chart-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; }
@media (max-width: 768px) { .chart-grid { grid-template-columns: 1fr; } }
.chart-wrap { background: var(--rp-card); border: 1px solid var(--rp-border); border-radius: 12px; padding: 20px; transition: all 0.2s; }
.chart-wrap:hover { border-color: var(--rp-border-hover); }
.chart-title { font-size: 14px; font-weight: 700; color: var(--rp-text); margin-bottom: 16px; display: flex; align-items: center; gap: 8px; }
.chart-title i { color: var(--rp-accent); }
...
.badge.good { background: rgba(34,197,94,0.1); color: var(--rp-pos); }
.badge.warn { background: rgba(234,179,8,0.1); color: var(--rp-warn); }
.badge.refund { background: rgba(139,92,246,0.1); color: #8b5cf6; }

/* ── Delta ── */
.delta { font-weight: 700; font-size: 11px; padding: 2px 6px; border-radius: 20px; }
.delta.up { color: var(--rp-pos); background: rgba(34,197,94,0.1); }
.delta.down { color: var(--rp-neg); background: rgba(239,68,68,0.1); }
.delta.neutral { color: var(--rp-text-muted); background: rgba(100,116,139,0.1); }

/* ── Report Summary ── */
.report-summary { font-size: 14px; line-height: 1.7; color: var(--rp-text-muted); padding: 16px; border-left: 3px solid var(--rp-accent); background: rgba(209,144,75,0.04); border-radius: 0 12px 12px 0; }
.report-summary strong { color: var(--rp-text); }
.report-summary strong.good { color: var(--rp-pos); }
.report-summary strong.ok { color: var(--rp-accent); }
.report-summary strong.warn { color: var(--rp-warn); }

/* ── Empty state ── */
.empty { text-align: center; padding: 60px 20px; color: var(--rp-text-muted); font-size: 14px; }
.empty i { font-size: 40px; display: block; margin-bottom: 12px; color: var(--rp-border); }

/* ── Tables ── */
.refund-table, .remake-table { width: 100%; border-collapse: collapse; font-size: 13px; }
.refund-table th, .remake-table th { text-align: left; padding: 10px 12px; font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: var(--rp-text-muted); border-bottom: 2px solid var(--rp-border); }
.refund-table td, .remake-table td { padding: 10px 12px; border-bottom: 1px solid var(--rp-border); color: var(--rp-text); }
.refund-table tbody tr:hover, .remake-table tbody tr:hover { background: rgba(209,144,75,0.03); }
.refund-table-wrap, .remake-table-wrap { overflow-x: auto; margin-top: 16px; }
...
.report-pager a, .report-pager span { padding: 6px 12px; border-radius: 8px; font-size: 13px; border: 1px solid var(--rp-border); color: var(--rp-text); text-decoration: none; transition: all 0.2s; }
.report-pager a:hover { border-color: var(--rp-accent); color: var(--rp-accent); }
.report-pager .active { background: var(--rp-accent); color: #fff; border-color: var(--rp-accent); }
...
/* Ã¢â€â‚¬Ã¢â€â‚¬ Custom Scrollbar Ã¢â€â‚¬Ã¢â€â‚¬ */
...
[data-theme="dark"] .card:hover {
    border-color: #d1904b;
    box-shadow: 0 4px 20px rgba(209,144,75,0.08);
}
...
/* Ã¢â€â‚¬Ã¢â€â‚¬ Refund KPI special styling Ã¢â€â‚¬Ã¢â€â‚¬ */
.kpi.refund-kpi {
...
/* Ã¢â€â‚¬Ã¢â€â‚¬ Table pagination Ã¢â€â‚¬Ã¢â€â‚¬ */
.report-pager {
...
/* Ã¢â€â‚¬Ã¢â€â‚¬ PC improvements Ã¢â€â‚¬Ã¢â€â‚¬ */
...
/* Ã¢â€â‚¬Ã¢â€â‚¬ Tablet Ã¢â€â‚¬Ã¢â€â‚¬ */
...
/* Ã¢â€â‚¬Ã¢â€â‚¬ Hourly 2-col Ã¢â€ â€™ 1-col on narrow screens Ã¢â€â‚¬Ã¢â€â‚¬ */
...
/* Ã¢â€â‚¬Ã¢â€â‚¬ INSIGHTS STRIP Ã¢â€â‚¬Ã¢â€â‚¬ */
...
/* Ã¢â€â‚¬Ã¢â€â‚¬ Live indicator Ã¢â€â‚¬Ã¢â€â‚¬ */
...
/* Ã¢â€â‚¬Ã¢â€â‚¬ Delta badges Ã¢â€â‚¬Ã¢â€â‚¬ */
...
/* Ã¢â€â‚¬Ã¢â€â‚¬ Daily goal progress bar Ã¢â€â‚¬Ã¢â€â‚¬ */
.goal-bar-wrap { margin-top:8px; }
...
<div class="top">
    <a class="back" href="/FinalSystem/pos-cafe/pages/dashboard/index.php">
...
<div class="card">
    ...
    <div class="kpis">
        ...
        <!-- Ã¢â€â‚¬Ã¢â€â‚¬ REFUND KPI Ã¢â€â‚¬Ã¢â€â‚¬ -->
        ...
        <!-- Ã¢â€â‚¬Ã¢â€â‚¬ REMAKE KPI Ã¢â€â‚¬Ã¢â€â‚¬ -->
        <div class="kpi" style="border-top: 3px solid #eab308;">
            <div class="kpi-icon" style="color:#eab308;">
                <i class="fa-solid fa-repeat"></i>
            </div>
            <span>Remakes</span>
            <h3 class="num" style="color:#eab308;"><?= $remakeCount ?></h3>
            <div class="small">
                <span class="badge warn"><i class="fa-solid fa-repeat"></i> drinks remade</span>
            </div>
        </div>

        <!-- Ã¢â€â‚¬Ã¢â€â‚¬ NET REVENUE KPI Ã¢â€â‚¬Ã¢â€â‚¬ -->
        <div class="kpi" style="border-top: 3px solid #22c55e;">
            <div class="kpi-icon">
                <i class="fa-solid fa-sack-dollar"></i>
            </div>
            <span>Take Home</span>
            <h3 class="num" id="kv-takehome" style="color:#22c55e;">$<?= fmtMoney($netRevenue) ?></h3>
            <div class="small">Sales minus refunds</div>
        </div>

        <?php if ($mode === 'daily' && $peakHour): ?>
        <!-- Ã¢â€â‚¬Ã¢â€â‚¬ PEAK HOUR KPI Ã¢â€â‚¬Ã¢â€â‚¬ -->
        <div class="kpi" style="border-top: 3px solid #d1904b;">
            <div class="kpi-icon">
                <i class="fa-solid fa-fire"></i>
            </div>
            <span>Busiest Hour</span>
            <h3 class="num" id="kv-peak" style="color:#d1904b; font-size:22px;"><?= $peakHour ?></h3>
...
<!-- Ã¢â€â‚¬Ã¢â€â‚¬ PERIOD SUMMARY Ã¢â€â‚¬Ã¢â€â‚¬ -->
<?php if ($orderCount > 0):
...
<div class="card" style="margin-bottom:16px;">
    ...
    <p class="report-summary" id="live-summary">
        During <strong><?= htmlspecialchars($label) ?></strong>, the cafÃƒÂ© completed <strong><?= $orderCount ?></strong> <?= $orderCount === 1 ? 'order' : 'orders' ?><?= $totalItemsSold > 0 ? ', serving <strong>' . $totalItemsSold . '</strong> items,' : '' ?> totalling <strong>$<?= fmtMoney($totalSales) ?></strong> in sales (avg <strong>$<?= fmtMoney($avgOrder) ?></strong> per order). After ingredient costs of <strong>$<?= fmtMoney($totalCOGS) ?></strong>, the gross profit was <strong>$<?= fmtMoney($totalProfit) ?></strong> Ã¢â‚¬â€ a margin of <strong class="<?= $marginClass ?>"><?= number_format($margin, 1) ?>%</strong>, which is <?= $marginHealth ?>.<?= $refundSentence ?><?= $peakSentence ?><?= $topProdSentence ?>
    </p>
</div>
<?php endif; ?>
...
<!-- Ã¢â€â‚¬Ã¢â€â‚¬ DAILY TREND (monthly / range mode) Ã¢â€â‚¬Ã¢â€â‚¬ -->
...
<div class="card">
    <div class="section-hdr">
        ...
        <p class="section-desc">Revenue and order count per day Ã¢â‚¬â€ spot your busiest days and slow periods across the selected range.</p>
    </div>
...
<!-- Ã¢â€â‚¬Ã¢â€â‚¬ HOURLY SALES (daily mode only) Ã¢â€â‚¬Ã¢â€â‚¬ -->
...
<div class="card">
    <div class="section-hdr">
        ...
        <p class="section-desc">Hourly revenue and order volume from 06:00Ã¢â‚¬â€œ22:00. Highlighted bar = highest-revenue hour. Right panel ranks top 5 hours.</p>
    </div>
...
<!-- Ã¢â€â‚¬Ã¢â€â‚¬ PAYMENT METHOD BREAKDOWN Ã¢â€â‚¬Ã¢â€â‚¬ -->
...
<div class="card">
    <div class="section-hdr">
        ...
        <p class="section-desc">How customers paid Ã¢â‚¬â€ useful for cash drawer reconciliation and understanding payment preferences.</p>
    </div>
...
<!-- Ã¢â€â‚¬Ã¢â€â‚¬ TOP PRODUCTS TABLE Ã¢â€â‚¬Ã¢â€â‚¬ -->
...
<div class="card">
    <div class="section-hdr">
        ...
        <p class="section-desc">Per-item revenue, ingredient cost (COGS), profit, and margin. Click any column header to sort. Green margin = Ã¢â€°Â¥50%, orange = Ã¢â€°Â¥25%, yellow = below 25%.</p>
    </div>
...
<!-- Ã¢â€â‚¬Ã¢â€â‚¬ NEW: REFUND CHART Ã¢â€â‚¬Ã¢â€â‚¬ -->
...
<div class="card">
    <div class="section-hdr">
        ...
        <p class="section-desc">Refund frequency over time Ã¢â‚¬â€ bars show count, line shows total amount refunded. Helps identify patterns or staff issues.</p>
    </div>
...
<!-- Ã¢â€â‚¬Ã¢â€â‚¬ NEW: REFUND LIST Ã¢â€â‚¬Ã¢â€â‚¬ -->
...
<div class="card">
    <div class="section-hdr">
        ...
        <p class="section-desc">Full list of refunds processed in this period Ã¢â‚¬â€ original amount, refund amount, stated reason, and who processed it.</p>
    </div>
...
<div class="report-section">
    ...
    <p class="section-desc">Drinks that were remade due to quality issues Ã¢â‚¬â€ useful for spotting recurring problems with specific drinks or baristas.</p>
        ...
        <table class="refund-table" id="remakeTable">
            ...
            <tbody>
                ...
                <tr>
                    <td>#<?= $r['daily_order_no'] ?></td>
                    <td><?= htmlspecialchars($r['customer_name']) ?></td>
                    <td style="color:var(--text);"><?= htmlspecialchars($r['products'] ?? 'Ã¢â‚¬â€') ?></td>
...
<script>
// Ã¢â€â‚¬Ã¢â€â‚¬ Apply saved theme immediately (prevents flash) Ã¢â€â‚¬Ã¢â€â‚¬
(function() {
...
// Ã¢â€â‚¬Ã¢â€â‚¬ Sync toggle button state on load Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬ Responsive chart helper Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬ Shared tooltip style Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬ Theme-aware chart tokens (re-read live so charts follow light/dark) Ã¢â€â‚¬Ã¢â€â‚¬
function CT() {
...
// Ã¢â€â‚¬Ã¢â€â‚¬ CATEGORY CHART Ã¢â€â‚¬Ã¢â€â‚¬
                    ...
                    ticks: {
                        ...
                        callback: function(value, index, ticks) {
                            ...
                            return label.length > charsAllowed
                                ? label.slice(0, charsAllowed - 1) + 'Ã¢â‚¬Â¦'
                                : label;
                        }
                    },
                    grid: { display: false },
...
// Ã¢â€â‚¬Ã¢â€â‚¬ TOP PRODUCTS CHART Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬ REFUND CHART Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬ HOURLY SALES CHART Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬ TOP REVENUE HOURS CHART Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬ DAILY TREND CHART (monthly / range mode) Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬ PAYMENT METHOD CHART Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬ EXPORT PDF Ã¢â€â‚¬Ã¢â€â‚¬
function exportPDF() {
    const params = new URLSearchParams(window.location.search);
    window.open('report_pdf.php?' + params.toString(), '_blank');
}

// Ã¢â€â‚¬Ã¢â€â‚¬ EXPORT CSV Ã¢â€â‚¬Ã¢â€â‚¬
function exportCSV() {
...
// Ã¢â€â‚¬Ã¢â€â‚¬ TABLE SORT Ã¢â€â‚¬Ã¢â€â‚¬
let sortState = { col: -1, dir: 1 };
// Ã¢â€â‚¬Ã¢â€â‚¬ Client-side table pagination Ã¢â€â‚¬Ã¢â€â‚¬
const __pagers = {};
...
function buildPagerControls(tableId, pages) {
    ...
    const btn = (label, target, o = {}) => o.ellipsis
        ? '<span class="rp-ellipsis">Ã¢â‚¬Â¦</span>'
        : `<button class="rp-btn${o.active ? ' active' : ''}" ${o.disabled ? 'disabled' : ''} data-go="${target}">${label}</button>`;
    let html = btn('Ã‚Â«', 1, { disabled: p === 1 }) + btn('Ã¢â‚¬Â¹', p - 1, { disabled: p === 1 });
    const win = [];
    ...
    html += btn('Ã¢â‚¬Âº', p + 1, { disabled: p === pages }) + btn('Ã‚Â»', pages, { disabled: p === pages });
...
// Ã¢â€â‚¬Ã¢â€â‚¬ Chart.js ready guard (needed when loaded async via AJAX embed) Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬ (Re)build every chart Ã¢â‚¬â€ reused by initial render, resize, and theme toggle Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬ Initial render Ã¢â€â‚¬Ã¢â€â‚¬
whenChartReady(rebuildAllCharts);

// Ã¢â€â‚¬Ã¢â€â‚¬ Re-render on resize (debounced) Ã¢â€â‚¬Ã¢â€â‚¬
let resizeTimer;
...
// Ã¢â€â‚¬Ã¢â€â‚¬ LIVE POLLING Ã¢â€â‚¬Ã¢â€â‚¬
...
function applyLiveData(d) {
    // Ã¢â€â‚¬Ã¢â€â‚¬ KPI numbers Ã¢â€â‚¬Ã¢â€â‚¬
    setEl('kv-orders',   String(d.orderCount));
    setEl('kv-avg',      'Avg $' + fmt2(d.avgOrder) + ' Ã‚Â· ' + (d.totalItemsSold ?? 0) + ' items');
    ...
    if (d.topProduct) {
        ...
        if (bsv) bsv.textContent = d.topProduct + ' Ã¢â‚¬â€ ' + d.topProductQty + ' sold';
    }
    if (d.topCategory) {
        const tcv = document.getElementById('ic-topcat-val');
        if (tcv) tcv.textContent = d.topCategory + ' Ã¢â‚¬â€ ' + d.topCategoryQty + ' items';
    }
    const mgv = document.getElementById('ic-margin-val');
    if (mgv) {
        mgv.textContent = d.margin + '% Ã¢â‚¬â€ ' + (d.margin >= 30 ? 'healthy' : 'below 30% target');
    ...
    if (d.peakHour) {
        ...
        if (pkv) pkv.textContent = d.peakHour + ' Ã¢â‚¬â€ most orders this hour';
    }
    if (d.refundCount > 0) {
        const rrv = document.getElementById('ic-refrate-val');
        if (rrv) rrv.textContent = d.refundRate + '% of orders Ã¢â‚¬â€ ' + d.refundCount + ' refunded';
    }
    ...
    if (sumEl && d.orderCount > 0) {
        ...
        sumEl.innerHTML = 'During <strong>' + sumEl.dataset.label + '</strong>, the cafÃƒÂ© completed <strong>' + d.orderCount + '</strong> ' + (d.orderCount === 1 ? 'order' : 'orders') + itemsNote + ' totalling <strong>$' + fmt2(d.totalSales) + '</strong> in sales (avg <strong>$' + fmt2(d.avgOrder) + '</strong> per order). After ingredient costs of <strong>$' + fmt2(d.totalCOGS) + '</strong>, the gross profit was <strong>$' + fmt2(d.totalProfit) + '</strong> Ã¢â‚¬â€ a margin of <strong class="' + marginCls + '">' + d.margin + '%</strong>, which is ' + marginHealth + '.' + refNote + pkNote + tpNote;
    }

    // Ã¢â€â‚¬Ã¢â€â‚¬ Hourly chart (daily mode only) Ã¢â€â‚¬Ã¢â€â‚¬
    ...
    // Ã¢â€â‚¬Ã¢â€â‚¬ Goal progress bar Ã¢â€â‚¬Ã¢â€â‚¬
    ...
    document.title = 'Report Ã¢â‚¬â€ $' + fmt2(d.totalSales) + ' Ã‚Â· CafÃƒÂ©';

    // Ã¢â€â‚¬Ã¢â€â‚¬ Timestamp Ã¢â€â‚¬Ã¢â€â‚¬
...
if (LIVE_MODE) {
    ...
    function _setDotState(ok) {
        ...
        if (ok) {
            ...
            if (ts && !ts.textContent.startsWith('reconnecting')) ts.textContent = 'reconnectingÃ¢â‚¬Â¦';
        }
    }
